...
CREATE TABLE PetOwner (
    ...
    owner_image_url VARCHAR(MAX) NULL,
...
CREATE TABLE VetClinic (
    ...
    clinic_photo_cloudinary_url VARCHAR(MAX) NULL,
    clinic_photo_reference VARCHAR(MAX) NULL,
...
CREATE TABLE MedicalRecord (
    ...
    record_consultation_type VARCHAR(100) NOT NULL,
    ...
    record_note VARCHAR(255) NULL,
    record_image VARCHAR(MAX) NULL,
    is_deleted BIT DEFAULT 0,
    created_at DATETIME DEFAULT GETDATE(),
    updated_at DATETIME DEFAULT GETDATE(),
    FOREIGN KEY (pet_id) REFERENCES Pet(pet_id) ON DELETE CASCADE,
    CHECK (CAST(record_visit_date AS DATE) <= CAST(GETDATE() AS DATE))
);
