import { Flex, Box, Text, Input, Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalFooter, Select } from "@chakra-ui/react";
...
export default function Calendar() {
  ...
  const tags = ["", "General Checkup", "Grooming", "Dental Care"];
  ...
  const saveEvent = () => {
    ...
    setEvents((prev) => {
      ...
      if (editingIndex !== null) {
        ...
        updatedList[editingIndex] = newEvent;
                ...
                return (
                  <Box key={i} w="calc(100% / 7 - 4px)" display="flex" justifyContent="center" alignItems="center" cursor="pointer" onClick={() => setSelectedDate(key)}>
                    <Box w="28px" h="28px" display="flex" justifyContent="center" alignItems="center" borderRadius="full" bg={ hasEvent ? "Primary.900" : isSelected ? "Primary.700" : isToday ? "Primary.300" : "transparent" }>
                      <Text fontSize="sm" color={hasEvent || isSelected ? "Neutral.100" : "Primary.900"}>
                        {day}
                      </Text>
                    </Box>
                  </Box>
        ...
        <Box w="340px" h="260px" borderRadius={10} bg="Primary.200" p="10px" overflowY="auto" display="flex" flexDirection="column" position="relative">
          ...
          {!selectedDate ? (
            ...
            selectedEvents.map((item, idx) => (
              <Flex key={idx} justify="space-between" align="center" mt={3} cursor="pointer" 
              onClick={() => {
                  setEditingIndex(idx);
              ...
              >
                <Flex direction="column">
                  {item.tag && (
                    <Box fontSize="xs" color="Primary.700">
                      {item.tag}
                    </Box>
                  )}
          ...
          <Box position="absolute" bottom="10px" right="10px">
            <Button w="40px" h="40px" minW="0px" p={0} borderRadius="100%" bg="Primary.800" isDisabled={!selectedDate} onClick={() => setIsOpen(true)}>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} isCentered>
        ...
        <ModalContent borderRadius="16px">
          ...
          <ModalFooter>
            ...
            <Button bg="Primary.800" color="Neutral.100" onClick={saveEvent}>
              Save
            </Button>
          </ModalFooter>
