import { Flex, Box, Text, Input, Textarea, Button, Image, Modal, ModalOverlay, ModalContent, IconButton, Menu, MenuButton, MenuList, MenuItem, useToast, InputGroup, InputRightElement } from "@chakra-ui/react";
...
export default function QuickNotes() {
  ...
  const handleSubmit = async () => {
    if (!selectedPet) {
      showToast("Please select a pet or create a new one!", "error");
      return;
    }
    if (!date || !type) {
      showToast("Please fill in the Examination Date and Consultation Type.", "error");
      return;
    }
    ...
    if (isNewPet && pet_dob > todayStr) {
      showToast("Invalid Date of Birth!", "error");
      return;
    }

    if (type === "Vaccination" && !photo) {
      showToast("A photo attachment is required for Vaccination records!", "error");
      return;
    }
    ...
    try {
      if (isNewPet) {
        if (!name || !typePet || !gender) {
          showToast("Please fill out the Name, Type, and Gender for the new pet.", "error");
          setIsLoading(false);
          return;
        }
    ...
    <Flex direction="column" p="20px" gap={4} minH="100vh" pb="120px" >
      ...
      {isNewPet && (
        <Flex direction="column" gap={4}>
          ...
          <Box>
            ...
            <Input placeholder="Pet name" bg="white" border="1px solid" borderColor="Primary.800" value={name} onChange={(e) => setName(removeEmojis(e.target.value))} />
          </Box>
      ...
      <Box bg="Primary.200" p="12px" borderRadius="10px" flex="1">
        ...
        <Input
          ...
          value={date}
          max={todayStr}
          onChange={(e) => setDate(removeEmojis(e.target.value))}
        />
      </Box>
      ...
      <Flex gap={4}>
        ...
        <Box bg="Primary.200" p="12px" borderRadius="10px" flex="1">
          ...
          <Input bg="Primary.100" border="none" color="Primary.800" value={temperature} onChange={(e) => setTemperature(sanitizeTemperature(e.target.value))} />
        </Box>
      </Flex>
